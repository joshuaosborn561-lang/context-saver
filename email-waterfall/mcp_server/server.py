...
mcp = MCPServer(
    ...
    description=(
        ...
        "Any snake_case client_tag works (ensure_client / auto-ensure). "
    ...
    return _json(
        {
            ...
            "auth": "none",
            "note": "Any snake_case client_tag works. Call ensure_client or auto-ensures on enrich_waterfall.",
        }
    )
...
def ensure_client(
    ...
    _ensure_repo_cwd()
    _reload_settings()
    from email_waterfall.supabase_sync import ensure_client as _ensure

    titles = [p.strip() for p in (target_titles or "").split(",") if p.strip()] or None
    cfg = _ensure(
        client_tag,
        display_name=display_name,
        profile=profile or "owner",
        icp=icp,
        titles=titles,
    )
    return _json(
        {
            "ok": True,
            "client_tag": cfg.tag,
            "display_name": cfg.owner or cfg.tag,
            "companies_table": f"public.{cfg.companies_table}",
            "contacts_table": f"public.{cfg.contacts_table}",
            "profile": cfg.profile,
            "target_titles": list(cfg.titles),
            "icp": cfg.icp,
        }
    )
...
def list_clients() -> str:
    ...
    _reload_settings()
    from email_waterfall.clients import list_registered_clients
    from email_waterfall.supabase_sync import list_clients_remote

    remote = list_clients_remote()
    if remote:
        return _json({"clients": remote, "n": len(remote)})
    local = list_registered_clients()
    return _json(
        {
            "clients": [
                {
                    "client_tag": c.tag,
                    "display_name": c.owner or c.tag,
                    "companies_table": c.companies_table,
                    "contacts_table": c.contacts_table,
                    "profile": c.profile,
                    "titles": list(c.titles),
...
def enrich_waterfall(
    ...
    client_tag is required (any snake_case tag). New tags auto-ensure
    public.{tag}_wf_companies / {tag}_wf_contacts.
    need = 'dm' | 'email' | 'both'.
    ...
    (owner profile) or pass explicitly for the run.
    ...
    # Auto-provision tables for any new snake_case tag (goliath, etc.).
    titles_override = [p.strip() for p in (target_titles or "").split(",") if p.strip()]
    try:
        client = _ensure(
            client_tag,
            profile="owner",
            titles=titles_override or None,
        )
    except Exception:
        # If RPC missing in a local/dev DB, still allow known/dynamic config.
        client = get_client(client_tag)
