...
Any snake_case `client_tag` works. `ensure_client` (or auto on `enrich_waterfall`) creates `{tag}_wf_companies` / `{tag}_wf_contacts`. Legacy: basco / peterson keep their original table names.
