...
from .clients import ClientConfig, build_client_config, normalize_client_tag, register_client
...
def ensure_client(
    ...
    *,
    ...
    """Create public.{tag}_wf_* tables (or legacy names) and register the client."""
    ...
    body: dict[str, Any] = {
        "p_client_tag": tag,
        "p_display_name": display_name or cfg.owner or tag,
        "p_profile": cfg.profile,
        "p_icp": icp or cfg.icp,
        "p_titles": list(cfg.titles),
    }
    status, raw = _request(
        "POST",
        "rpc/ew_ensure_client",
        body=body,
        prefer="return=representation",
    )
    if status >= 400:
        raise RuntimeError(f"ew_ensure_client failed ({status}): {raw[:500]}")
    register_client(cfg)
    return cfg


def list_clients_remote() -> list[dict[str, Any]]:
    try:
        status, raw = _request(
            "POST",
            "rpc/ew_list_clients",
